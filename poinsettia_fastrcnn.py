...
using_google_collab = True

root_path = ''
if using_google_collab:
  from google.colab import drive
  drive.mount('/content/gdrive', force_remount=True)
  root_path = 'gdrive/My Drive/AgriForwards/MSc/Poinsettia Challenge/top-view/'
...
class PoinsettiaDataset(object):
  ...
  """
  def __init__(self, root, img_folder,transforms=None):
    self.root = root
    self.transforms = transforms
  ...
  def __getitem__(self, idx):
    ...
    target['image_id'] = torch.Tensor([img_id for _ in range(len(label_tensor))])
    ...
    target['masks'] = self.create_mask(img, target['boxes'])
...
def main():
  ...
  dataset_test = PoinsettiaDataset(root_path, 'test', get_transforms(False))
    ...
    with torch.no_grad():
      if epoch % 5 == 0 or epoch == num_epochs-1:
        ...
        for j, test_data in enumerate(dataloader_test):
          ...
          ax.imshow(imgs)
          out = outputs
          for each in out[0]['boxes']:
            each = each.detach().cpu()
            width = int(each[2]-each[0])
            height = int(each[3]-each[1])
            rect = patches.Rectangle((each[0], each[1]), width, height, linewidth=1,edgecolor='r',facecolor='none')    
            ax.add_patch(rect)
          plt.show()
